"""Fix id columns from uuid to text
...
def upgrade() -> None:
    op.execute("ALTER TABLE user_tokens ALTER COLUMN id TYPE text USING id::text")
    op.execute("ALTER TABLE connected_accounts ALTER COLUMN id TYPE text USING id::text")


def downgrade() -> None:
    op.execute("ALTER TABLE user_tokens ALTER COLUMN id TYPE uuid USING id::uuid")
